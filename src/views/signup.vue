<template>
  <div class="min-w-full min-h-screen flex items-center justify-center px-2 py-5">
    <div class="bg-gray-100 text-black-500 rounded-3xl shadow-xl w-full overflow-hidden" style="max-width:1000px">
      <div class="md:flex w-full">
        <div class="hidden md:block w-1/2 bg-indigo-600 py-10 px-10">
          <svg id="a87032b8-5b37-4b7e-a4d9-4dbfbe394641" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg"
            ...
            <path id="b3ac2088-de9b-4f7f-bc99-0ed9705c1a9d" data-name="Path 22"
                  ...
                  transform="translate(-227.576 -76.46149)" fill="#fffff"/>
        ...
        <div class="w-full md:w-1/2 pt-6 pb-6 px-5 md:px-10 md:py-10">

          <!--          alert of empty email password  +++++++++++++++++++++++++++++++++++++-->
          <div class="max-w-lg mx-auto"
               v-for="error in signupErrors">
            <div class="flex bg-red-100 rounded-lg px-4 py-2 mb-2 text-sm text-red-700" role="alert">
          ...
          <div class="max-w-lg mx-auto"
            ...
            <div class="flex bg-red-100 rounded-lg p-4 mb-4 text-sm text-red-700" role="alert">
              ...
              <div>
                <span class="font-medium">{{ compteExist }}</span>
              </div>
            </div>
          </div>
          <!--            compte added succefully  +++++++++++++++++++++++++++++++++++++-->
          <div class="max-w-lg mx-auto"
               v-if="compteCreated">
            <div class="flex bg-green-100 rounded-lg p-4 mb-4 text-sm text-green-700" role="alert">
              ...
              <div>
                <span class="font-medium">{{ compteCreated }}</span>
              </div>
            </div>
          </div>
          ...
          <form>
            ...
            <div class="flex -mx-3">
              <div class="w-full px-3 mb-5">
                ...
                <div class="flex">
                  ...
                  <input type="email"
                         class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-cyan-500"
                         placeholder="ahimbaj@example.com"
                         v-model="email">
                </div>
              </div>
            </div>
            ...
            <div class="flex -mx-3">
              <div class="w-full px-3 mb-5">
                <button
                    @click="signup"
                    class="block w-full max-w-xs mx-auto bg-cyan-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">
                  REGISTER NOW
                </button>
              </div>
            </div>
            <div class="flex -mx-3">
              <div class="w-full px-3 mb-5 text-center text-sm font-semibold">
                Do you have an account ? <a class="cursor-pointer text-indigo-600 hover:text-indigo-800">
...
export default {
  ...
  methods: {
    validateSignup() {
      ...
      this.signupErrors = [];
      if (!this.email) {
        this.signupErrors.push("email can not be empty");
        valid = false;
      }
      if (!this.password) {
        this.signupErrors.push("password can not be empty");
        valid = false;
      }
      return valid;
    },
    async signup() {
      ...
      if (valid) {
        await createUserWithEmailAndPassword(
            ...
            (credential) => {
              this.compteCreated = 'Successfully signed up';
              ...
              if(err.code==='auth/email-already-in-use'){
                this.compteExist = 'An account with this email already exists. Please log in.';
              }else {
                this.compteExist='An error occurred during signup. Please try again later.';
